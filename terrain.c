...
SDL_FColor determine_color_from_height(float y, range hr);
...
int main(int argc, char *argv[])
{
    ...
    // mesh parameters
    int mesh_size = 60;
    float mesh_step = 0.15f;
    float mesh_length = mesh_step * mesh_size;
    int t_num = 2 * mesh_size * mesh_size;

    // mountains parameters
    mtype_parameters big = {1, 3, 1.2, 2.0, 0.1, 1.2, 0.05};
    mtype_parameters med = {5, 9, 0.5, 1.2, 1.2, 2.5, 0.15};
    mtype_parameters sml = {7, 16, 0.05, 0.5, 2.5, 5, 0.3};
    int max_m_num = big.num_max + med.num_max + sml.num_max;
    int m_num = 0;
    ...
    // buffers
    triangle *t_buffer = malloc(sizeof(triangle) * t_num);
    ...
    while (is_running)
    {
        ...
        for (int ti = 0; ti < t_num; ti++)
        {
            ...
            ft.d = avg_depth(&t);
            ft.color = determine_color_from_height(bigest_height_value((terrain + ti)), hr);
            ft_buffer[ti] = ft;
        }
        qsort(ft_buffer, t_num, sizeof(triangle_flat), compare_triangle_depth);
        if (!fill)
        {
            for (int ti = 0; ti < t_num; ti++)
            {
        ...
        if (fill)
        {
            ...
            SDL_RenderGeometry(renderer, NULL, v_buffer, t_num * 3, NULL, t_num * 3);
        }
...
mountain mountain_rand(mtype_parameters *p, float mesh_length)
{
    ...
    t.h = randf(p->h_min, p->h_max);
    t.sharp = randf(p->sharp_min, p->sharp_max);
    if (randf(0.0f, 1.0f) <= p->reverse_prob)
    {
        t.h *= -1.0f;
    }
    return t;
...
{
    int mi = 0;
    int num_big = rand() % (big->num_max - big->num_min + 1) + big->num_min;
    int num_med = rand() % (med->num_max - med->num_min + 1) + med->num_min;
    int num_sml = rand() % (sml->num_max - sml->num_min + 1) + sml->num_min;
...
SDL_FColor determine_color_from_height(float y, range hr)
{
    ...
    float r[] = {0.00f, 0.20f, 0.60f, 1.00f, 1.00f, 1.00f, 1.00f};
    float g[] = {0.00f, 0.00f, 0.00f, 0.10f, 0.50f, 0.85f, 1.00f};
    float b[] = {0.00f, 0.30f, 0.50f, 0.20f, 0.00f, 0.00f, 1.00f};
    int n = 6;
